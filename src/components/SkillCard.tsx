...
export default function SkillCard({ name, src, description, expertise, index }: SkillCardProps) {
  ...
  return (
    <motion.div
      className={`shadow-neumorph p-6 rounded-lg group relative overflow-hidden bg-white dark:bg-gray-800 ${
        expertise > 3 ? 'border-2 border-transparent relative' : ''
      }`}
    ...
    >
      ...
      {expertise !== undefined && (
        <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5 mt-2 mb-3 overflow-hidden relative">
          <motion.div
            className="absolute inset-0 bg-gradient-to-r from-blue-400/20 via-purple-500/20 to-pink-400/20 opacity-50 blur-sm"
            initial={{ width: 0 }}
      ...
      {/* Hover content */}
      <motion.div 
        className="absolute inset-0 bg-white dark:bg-gray-800 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300" 
        initial={false}
      >
        <div className="p-4 h-full flex flex-col justify-between">
          ...
          <motion.a
            ...
            className="inline-flex items-center text-blue-500 hover:text-blue-600 dark:text-blue-400 dark:hover:text-blue-300 text-sm font-medium mt-2"
            whileHover={{ x: 5 }}
          >
