...
export const CVDownloadSection: React.FC<CVDownloadSectionProps> = ({ isDark }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 md:py-12 my-8 md:my-12 px-4 md:px-0">
      ...
      <AnimatePresence>
        {isModalOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black bg-opacity-75 backdrop-blur-sm z-50 flex items-center justify-center p-2 md:p-4">
              ...
              <div className="relative">
                {loadingPdf && (
                  <div className="absolute inset-0 flex items-center justify-center bg-gray-50 dark:bg-gray-900">
                    <div className="w-8 h-8 md:w-10 md:h-10 border-4 border-blue-500 border-t-transparent rounded-full animate-spin" />
                  </div>
                )}
